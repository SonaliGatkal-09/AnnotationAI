"use strict";
(self["webpackChunkanotate_imagesimilaritysearch"] = self["webpackChunkanotate_imagesimilaritysearch"] || []).push([[2761],{
...
var index = isClient ? index_js_.useLayoutEffect : noop;
...
function useLatestRef(value) {
  const ref = index_js_.useRef(value);
  index(() => {
    ref.current = value;
  });
  return ref;
...
function useFloating(options) {
  ...
  const [data, setData] = index_js_.useState({
    x: 0,
    y: 0,
    strategy,
    placement,
  ...
  const [latestMiddleware, setLatestMiddleware] = index_js_.useState(middleware);
  if (!deepEqual(latestMiddleware, middleware)) {
    setLatestMiddleware(middleware);
  }
  const [_reference, _setReference] = index_js_.useState(null);
  const [_floating, _setFloating] = index_js_.useState(null);
  const setReference = index_js_.useCallback(node => {
    if (node !== referenceRef.current) {
      referenceRef.current = node;
      _setReference(node);
    }
  }, []);
  const setFloating = index_js_.useCallback(node => {
  ...
  const referenceRef = index_js_.useRef(null);
  const floatingRef = index_js_.useRef(null);
  const dataRef = index_js_.useRef(data);
  const hasWhileElementsMounted = whileElementsMounted != null;
  const whileElementsMountedRef = useLatestRef(whileElementsMounted);
  const platformRef = useLatestRef(platform);
  const openRef = useLatestRef(open);
  const update = index_js_.useCallback(() => {
  ...
  const isMountedRef = index_js_.useRef(false);
  index(() => {
  ...
  const floatingStyles = index_js_.useMemo(() => {
    const initialStyles = {
      position: strategy,
      left: 0,
      top: 0
...
var dist = __webpack_require__(8342);
...
var NAME = "Arrow";
var Arrow = index_js_.forwardRef((props, forwardedRef) => {
...
var react_use_size_dist = __webpack_require__(9330);
...
var PopperAnchor = index_js_.forwardRef(
  (props, forwardedRef) => {
    ...
    return virtualRef ? null : /* @__PURE__ */ (0,jsx_runtime.jsx)(dist/* Primitive */.sG.div, { ...anchorProps, ref: composedRefs });
  }
);
PopperAnchor.displayName = ANCHOR_NAME;
var CONTENT_NAME = "PopperContent";
var [PopperContentProvider, useContentContext] = createPopperContext(CONTENT_NAME);
var PopperContent = index_js_.forwardRef(
  (props, forwardedRef) => {
    const {
      __scopePopper,
      side = "bottom",
    ...
    const [arrow, setArrow] = index_js_.useState(null);
...
var PopperArrow = index_js_.forwardRef(function PopperArrow2(props, forwardedRef) {
